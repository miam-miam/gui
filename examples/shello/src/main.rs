...
use gui::vello::util::{RenderContext, RenderSurface};
use gui::vello::Renderer;
use gui::vello::{
...
use crate::common::text::ParleyBrush;
use gui::glazier::kurbo::Size;
use gui::glazier::{
...
fn main() {
    ...
    let window = gui::glazier::WindowBuilder::new(app.clone())
...
pub fn render_anim_frame(scene: &mut Scene, fcx: &mut FontContext, i: u64) {
    ...
    let mut lcx = gui::parley::LayoutContext::new();
    let mut layout_builder =
        lcx.ranged_builder(fcx, "Hello vello! ഹലോ ਸਤ ਸ੍ਰੀ ਅਕਾਲ مرحبا!", scale as f32);
    layout_builder.push_default(&gui::parley::style::StyleProperty::FontSize(34.0));
    layout_builder.push(
        &gui::parley::style::StyleProperty::Brush(ParleyBrush(Brush::Solid(Color::rgb8(
            255, 255, 0,
        )))),
        6..10,
    );
    layout_builder.push(&gui::parley::style::StyleProperty::FontSize(48.0), 6..12);
    layout_builder.push(
        &gui::parley::style::StyleProperty::Brush(ParleyBrush(Color::YELLOW.into())),
        6..12,
    );
    layout_builder.push_default(&gui::parley::style::StyleProperty::Brush(ParleyBrush(
        Brush::Solid(Color::rgb8(255, 255, 255)),
    )));
    let mut layout = layout_builder.build();
    layout.break_all_lines(None, gui::parley::layout::Alignment::Start);
    common::text::render_text(&mut sb, Affine::translate((100.0, 400.0)), &layout);
